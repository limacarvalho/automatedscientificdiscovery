...
class Dimension_reduce_main:
    ...
    def __init__(self, params: dict, docu=False):
        ...
        self.docu = docu
        logging.info(f"FUNCTIONS: {self.dimred_functions}")

    def best_functions_step2(self, results_step1: list, results_step2: list) -> list:
        """
    ...
    def main(self) -> (int, dict, pd.DataFrame):
        ...
        """
        if self.docu:
            from analysis.dataframe_results_dimred import \
                Dataframe_results_dimred
            from analysis.hyperparameter_analysis import \
                main_analysis_hyperparameters
            from dimension_tools.dimension_suite.extra.plots.plot_q_coranking import \
                Figure_q_plots
        ...
        Target_dim = Target_dimension(
            data_high=data_scale, data_high_small=data_small, data_id=self.data_id, cutoff_loss=self.cutoff_loss
        )
        ...
        results_step2 = ray_get_optimization(
            functions=self.dimred_functions, data_high=data_small, dim_low=target_dim, cutoff_loss=self.cutoff_loss
        )
        ...
        results_step3 = dimreduce_full_data(cutoff_loss=self.cutoff_loss).intrinsic_dimension_full_data(
            lods_best_functions=lods_best_functions, data_high=data_scale, dim_low=target_dim
        )
