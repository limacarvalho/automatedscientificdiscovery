...
from helper_data.utils_data import check_low_dimension, empty_dict
...
class Hyperparameter_optimization:
    ...
    def black_box_dimreduce(self, **params: dict) -> float:
        ...
        try:
            ...
            logging.error("adding init steps or iter steps to dict")
    ...
    def get_init_iterations(self, hyperparameters: dict) -> (int, int):
        ...
        try:
            ...
            init, iterations = 1, 0
            logging.error(f"{self.fun_id} n_hyperpars: {str(n_hps)}")
        logging.info(f"{self.fun_id} n_hyperpars: {str(n_hps)} init: {str(init)} iterations: {str(iterations)}")
    ...
    def fun_maximize_optimizer(self, hyperparameters: dict) -> object:
        ...
        :return: objec,
            optimizer.max
        """
        optimizer = BayesianOptimization(f=self.black_box_dimreduce, pbounds=hyperparameters, random_state=1, verbose=0)
        try:
            optimizer.maximize(init_points=self.init_steps, n_iter=self.iter_steps)
        except:
            logging.info(f"no loss change with hyperparameter tuning {self.fun_id}")
            pass
        return optimizer
    ...
    def get_best_optimizer_result(self, optimizer):
        ...
        try:
            ...
            best_result = {}
            logging.error(f"find best results {self.fun_id}")
        return best_result

    def hp_optimization_bayes(self, fun_id: str, hyperparameters: dict, data: np.array, ndim: int) -> (dict, list):
        """
        - - - DESCRIPTION - - -
