...
class R_helper:
    ...
    def r_error_handler(self, fun_id: str):
        ...
        if os.path.exists(self.path_error_file):
            ...
            logging.error(f"R {fun_id} {file_contents}")
            f.close()
            if os.path.exists(self.path_error_file):
    ...
    def r_command_string(self, fun_id: str, default_parameters: str, arg_string: str, dim_low: int) -> str:
        ...
        command_string = str(
            ...
            + str('}, error = function(e){ write.table(as.character(e), file = "' + self.path_error_file + '")})')
        )
        ...
        logging.info(f"start dimreduction: {command_string}")
        return command_string

    def r_function_exe(
        self, fun_id: str, default_parameters: str, arg_string: str, data_high: np.array, dim_low: int
    ) -> (np.array, dict):
        """
    ...
    def argstring_R_to_dict(self, arg_string: str) -> dict:
        ...
        try:
            ...
            dict_argstring = {"empty": "empty"}
            logging.error("params datatype must be string or dict")
