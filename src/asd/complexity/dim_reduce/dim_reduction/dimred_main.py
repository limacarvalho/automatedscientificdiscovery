...
def call_dimred_functions(custom_functions: Union[str, list], data_high: np.array) -> dict:
    ...
    # message strings
    string_except = (
        "choosing this dimred function(s) failed.\n"
    ...
    if custom_functions[0] == "All":
        try:
            ...
            logging.error("error in choosing functions: all functions")
        ...
        if exclude:
            try:
                ...
                logging.info(f"excluding functions: {str(custom_functions)}")
                return dict_funs
            except:
                logging.error(string_except)
            ...
            try:
                ...
                for fun_id in custom_functions:
                    ...
                    # function is not found
                        ...
                        logging.error(f"{fun_id} {string_except}")
                # make sure something is returned, if not stopp
                if dict_funs:
                    return dict_funs
                else:
                    logging.critical(string_except)
            except:
                logging.error(string_except)
...
class Dimreduction:
        ...
        try:
            ...
            if step == globalstring_step2:
                params_r_str, params_py_dict = hyperparameter_init(
                    params=params, fun_id=self.fun_id, dim=int(self.dim_low), data_high=self.data_high
                )
            ...
            logging.error(f"INIT HYPERPARAMETER {step} {self.fun_id} dim: {str(self.dim_low)}")
    ...
    def exe_dimreduce(self, params: dict, step: str) -> (np.array, dict):
        ...
        try:
            ...
            if isinstance(self.params_fun, str):
                data_low, hyperparameters = R_helper().r_function_exe(
                    self.fun_id, self.params_fun, params_r_str, self.data_high, self.dim_low
                )
            # Python functions
            else:
            ...
            logging.error(f"DIMREDUCE {step} {self.fun_id} dim: {str(self.dim_low)}")
        ...
        try:
            ...
            logging.error(" adding step to dict, add whitespace instead")
            dict_results["step"] = " "
        ...
        try:
            ...
            logging.error(" adding fun_id to dict, add empty instead")
        ...
        try:
            ...
            logging.error(" adding execution time to dict, add 0 instead")
            dict_results["time"] = 0
        ...
        try:
            ...
            logging.error(" adding rows_cols to dict, add 0 instead")
        ...
        try:
            ...
            logging.error(" adding dim_low to dict")
        ...
        try:
            ...
            logging.error(" adding hyperparamers-string to dict, add empty string")
        ...
        if step == globalstring_step3:
            try:
                ...
                logging.error("adding data_lowdim to dict, add NaN instead")
                dict_results["data_lowdim"] = "NaN"

        if step == globalstring_step3:
            logging.info(
                f"finito: {self.fun_id} dim: {str(self.dim_low)} loss: {str(dict_results[globalvar_loss_function])} time: {str(stop)}"
            )
        return dict_results
