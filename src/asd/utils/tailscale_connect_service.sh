...
# Enforces that DNS resolv.conf always contains tailscale MagicDNS and Google Public DNS Server
update_dns() {
    cat << EOFMAIN > /etc/resolv.conf
...
nameserver 8.8.8.8
search asd.localdomain
EOFMAIN
}

# Forces DNS before Tailscale connection
update_dns

# # Determines if '.asd_container_uuid' file exists and sets the tailscale hostname based on whether the script is executed in the main ASD docker container or in an EC2 instance
asd_container_uuid_file=/root/.asd_container_uuid
if [ -f "$asd_container_uuid_file" ]; then
    tailscale_hostname=main-asd
else
    tailscale_hostname=ray-node
fi
echo "+++ Set tailscale hostname to: $tailscale_hostname +++"
...
# Calls tailscale function
tailscale

# Forces DNS after Tailscale connection
update_dns