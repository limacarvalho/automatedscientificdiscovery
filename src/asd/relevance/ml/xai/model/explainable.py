...
from relevance.utils import helper
...
def __get_gs_attr__(model, df_X, stdevs, n_samples):
    ...
    gs_attr, delta = gs.attribute(
        df_X_tensor, stdevs=stdevs, n_samples=n_samples, baselines=baseline_dist, return_convergence_delta=True
    )
...
class Explainable:
    ...
    def get_attr(self, attr_algos):
        ...
        logging.info("attribution methods  " + str(attr_algos))

        for base_model in self.ensemble_set.base_models:
            logging.info("calculating variable importance on  " + str(base_model.model_file_name))

            for attr_algo in attr_algos:
                if type(base_model.gs.best_estimator) == nn.Sequential:
                    if attr_algo == "ig":
                        lazy_results.append(
                            __get_ig_attr__.remote(base_model.gs.best_estimator, df_X_id, self.ig_n_steps)
                        )
                        col_names.append(base_model.model_file_name + "_" + attr_algo)
                    elif attr_algo == "shap":
                        lazy_results.append(
                            __get_shapley_torch_attr__.remote(
                                base_model.gs.best_estimator, df_X_id, self.shapley_n_background
                            )
                        )
                        col_names.append(base_model.model_file_name + "_" + attr_algo)
                    elif attr_algo == "gradientshap":
                        lazy_results.append(
                            __get_gs_attr__.remote(base_model.gs.best_estimator, df_X_id, self.stdevs, self.n_samples)
                        )
                        col_names.append(base_model.model_file_name + "_" + attr_algo)
                elif (
                    type(base_model.gs.best_estimator)
                    in [
                    ...
                    type(base_model.gs.best_estimator)
                    in [KNeighborsRegressor, KNeighborsClassifier, BaggingClassifier, BaggingRegressor]
                ) & (attr_algo == "shap"):
                    lazy_results.append(
                        __get_shapley_kernel_attr__.remote(
                            base_model.gs.best_estimator, df_X_id, self.shapley_n_background
                        )
                    )
                    col_names.append(base_model.model_file_name + "_" + "shap")
    ...
    def __beautify_scores__(self, attr_algos, results, col_names):
        ...
        for col in df_results:
            if col != "cols":
                df_scores_ranked[col] = df_results[col].rank(
                    na_option="bottom", ascending=True, method="max", pct=False
                )
