...
import pandas as pd
import ray
from relevance.ml.models.base.tune import (BriskBagging, BriskKNN,
                                           BriskXGBoost, SlugLGBM, SlugRF,
                                           SlugXGBoost)
...
class Ensemble:
    def __init__(
        ...
        self.ensemble = BriskBagging(
            name="ensemble_brisk_bagging",
            pred_class=pred_class,
    ...
    def _fetch_models_pll(self, X_train, X_test, y_train, y_test, threshold=None):
        ...
        logging.info("Ensemble: starting discovery process for models " + str(self._base_models))
    ...
    def select(self, threshold=None):
        ...
        logging.info("filtered out " + str(len(self._base_models) - len(self.base_models)) + " models.")
    ...
    def fit(self, X_train, X_test, y_train, y_test):
        ...
        if self.base_models is None:
            logging.info("Fit base models first.")
            return None
        ...
        logging.info("Ensemble: fiting ensemble on " + str(df_X_train_ensemble.shape[1]) + " models")
