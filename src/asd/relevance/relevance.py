...
import pandas as pd
from relevance.ml.models import Ensemble, list_base_models
from relevance.ml.xai.model import Explainable, list_xai_algos
from relevance.ml.xai.non_model import list_fstats, simulate_knockoffs
...
def relevance(df, input_columns, target, options) -> None:
    ...
    if base_models:
        ...
        if ("briskxgboost" in base_models) or ("slugxgboost" in base_models):
            if "xgb_objective" in options:
                if options["xgb_objective"]:
                    ...
                    raise ValueError(
                        "Pls select objective function for xgboost, i.e., xgb_objective='reg:squarederror', https://xgboost.readthedocs.io/en/stable/parameter.html"
                    )
            else:
                raise ValueError(
                    "Pls select objective function for xgboost, i.e., xgb_objective='reg:squarederror', https://xgboost.readthedocs.io/en/stable/parameter.html"
        ...
        if "sluglgbm" in base_models:
            if "lgbm_objective" in options:
                if options["lgbm_objective"]:
                    ...
                    raise ValueError(
                        "Pls select objective function for xgboost, i.e., lgbm_objective='regression_l1', https://lightgbm.readthedocs.io/en/latest/Parameters.html"
                    )
            else:
                raise ValueError(
                    "Pls select objective function for xgboost, i.e., lgbm_objective='regression_l1', https://lightgbm.readthedocs.io/en/latest/Parameters.html"
        ...
        if "score_func" in options:
            if options["score_func"]:
                ...
                raise ValueError(
                    "Pls specify a scorer, i.e., r2_score/'r2_', log_loss/'neg_log_loss', etc., See https://scikit-learn.org/stable/modules/model_evaluation.html"
                )
        else:
            raise ValueError(
                "Pls specify a scorer, i.e., r2_score/'r2_', log_loss/'neg_log_loss', etc., See https://scikit-learn.org/stable/modules/model_evaluation.html"
        ...
        if "ensemble_bagging_estimators" in options:
            ...
            if ensemble_bagging_estimators < 11:
                raise ValueError(
                    "ensemble_bagging_estimators > 0 required, you specified: " + str(ensemble_bagging_estimators)
                )
        else:
            ...
            if "knockoffs" in attr_algos:
                ...
                if "knockoff_runs" in options:
                    if options["knockoff_runs"]:
                        ...
                        if knockoff_runs < 0:
                            raise ValueError(
                                "knockoff_runs > 0 required, you specified, we recommend knockoff_runs=20000 "
                                + str(knockoff_runs)
                            )
                    else:
                        knockoff_runs = 20000
    ...
    try:
        ...
        if "knockoffs" in attr_algos:
            logging.info("Running knockoffs with the following setting:")
            logging.info("     knockoff_runs:" + str(knockoff_runs))
            logging.info("     fdr:" + str(fdr))
            logging.info("     fstats:" + str(fstats))
        ...
        logging.info("Running Ensemble Modeling with following setting:")
        logging.info("     base models: " + str(base_models))
        logging.info("     pred_class: " + str(pred_class))
        logging.info("     xgb_objective: " + str(xgb_objective))
        logging.info("     lgbm_objective:" + str(lgbm_objective))
        logging.info("     score_func: " + str(score_func))
        logging.info("     metric_func: " + str(metric_func))
        logging.info("     n_trials: " + str(n_trials))
        logging.info("     boosted_round: " + str(boosted_round))
        logging.info("     max_depth: " + str(max_depth))
        logging.info("     rf_n_estimators: " + str(rf_n_estimators))
        logging.info("     bagging_estimators: " + str(bagging_estimators))
        logging.info("     n_neighbors: " + str(n_neighbors))
        logging.info("     cv_splits: " + str(cv_splits))
        logging.info("     ensemble_bagging_estimators: " + str(ensemble_bagging_estimators))
        logging.info("     ensemble_n_trials: " + str(ensemble_n_trials))
        ...
        logging.info("base model search started")
        ensemble_set.fetch_models(X_train_scalar, X_test_scalar, y_train, y_test, threshold=None)
        logging.info("base model scores" + str(ensemble_set.base_model_scores))

        logging.info("running xai on trained models")
