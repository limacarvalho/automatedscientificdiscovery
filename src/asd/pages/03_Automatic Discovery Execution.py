...
if st.session_state["discovery_type"] == "Predictability":
    ...
    if pred_target_column and pred_primekey_cols and pred_col_set and pred_ml_method and pred_output_column:
        ...
        if st.button("Get combinations", on_click=pred_combinations_click) or st.session_state["pred_get_combinations"]:
            ...
            get_column_combinations = asdpu.get_column_combinations(all_cols=predict_num_columns_ml.columns, inputs=pred_input_column, outputs=pred_output_column, targets=pred_target_column_ext, amount_only=False, return_targets=False)
        ...
        if st.button("Start discovery", on_click=pred_discovery_click) or st.session_state["button_pred_start_discovery"]:
            ...
            if pred_plot_along:
                if pred_refined_n_best == 0:
                    pred_output = asdpu.plot_result(input_datas_dict=datas_dict, plot_comb=struc_dict, refined_plot=False, refined_input_datas_dict=None, plot_along=pred_plot_along)
                    st.write(pred_output)
                else:
                    pred_output = asdpu.plot_result(input_datas_dict=datas_dict, plot_comb=struc_dict, refined_plot=True, refined_input_datas_dict=None, plot_along=pred_plot_along)
                    st.write(pred_output)
    ...
    if complex_column and complex_ident and complex_cutoff_loss:
        ...
        if st.button("Start discovery", on_click=complexity_discovery_click) or st.session_state["button_complexity_start_discovery"]:
            ...
            def complexity_discovery():
                intrinsic_dimension, best_results, df_summary = complexity.complexity(complex_data_high, str(complex_ident), complex_column, float(complex_cutoff_loss), complex_ml_method)
