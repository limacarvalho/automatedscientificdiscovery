...
def get_column_combinations(all_cols, inputs, outputs, targets=[], amount_only=False, return_targets=False):
    ...
    try:
        ...
        logging.error("Assertion failed: More input and output columns specified than there are columns.")
        raise err
    #
    # outputs vs. targets
    if targets:
        try:
            ...
            logging.error("Assertion failed: More output columns specified than there are in the targets list.")
            raise err
    ...
    # only get the amount of combination tuples according to the formulae in the docstring description
        ...
        if targets:
            amount_combinations = comb(len(all_cols) - len(targets), inputs, exact=True) * comb(
                len(targets), outputs, exact=True
            )
        else:
            amount_combinations = comb(len(all_cols), inputs, exact=True) * comb(
                len(all_cols) - inputs, outputs, exact=True
            )
...
def data_prep_split(data, input_cols, output_cols, random_state_split):
    ...
    # train test split
    curr_X_train, curr_X_test, curr_y_train, curr_y_test = train_test_split(
        curr_x, curr_y, test_size=0.3, shuffle=True, random_state=random_state_split
    )
...
def parallel_pred_step_MLP(
    ...
    logging.info(f"Analysing {str(curr_tuple)} now.")
    ...
    curr_X_train, curr_X_test, curr_y_train, curr_y_test = data_prep_split(
        curr_data, curr_inputs, curr_outputs, random_state_split
    )
    ...
    #
    if (
        ((curr_X_train > 0).all().all())
        and ((curr_X_test > 0).all().all())
        and ((curr_y_train > 0).all().all())
        and ((curr_y_test > 0).all().all())
    ):
        do_pl_fit = True
    else:
        do_pl_fit = False
    ...
    if scaling == "yes":
        pipe = Pipeline([("scaler", StandardScaler()), ("mlp", MLPRegressor(max_iter=max_iter))])
        pipe_params = [{"mlp__hidden_layer_sizes": hidden_layers, "mlp__alpha": alphas}]
        clf = GridSearchCV(
            pipe, param_grid=pipe_params, cv=3, scoring=scoring, return_train_score=True, verbose=verbose, n_jobs=n_jobs
        )
    elif scaling == "no":
        pipe = Pipeline([("scaler", StandardScaler()), ("mlp", MLPRegressor(max_iter=max_iter))])
        pipe_params = [{"scaler": ["passthrough"], "mlp__hidden_layer_sizes": hidden_layers, "mlp__alpha": alphas}]
        clf = GridSearchCV(
            pipe, param_grid=pipe_params, cv=3, scoring=scoring, return_train_score=True, verbose=verbose, n_jobs=n_jobs
        )
    else:
        if scaling != "test":
            logging.error(
                f"The specified usage of scaling ('{scaling}') is not an allowed option. Allowed options "
                "are 'yes', 'no', 'test'. Using 'test' now."
            )

        pipe = Pipeline([("scaler", StandardScaler()), ("mlp", MLPRegressor(max_iter=max_iter))])
        pipe_params = [
            {"scaler": ["passthrough"], "mlp__hidden_layer_sizes": hidden_layers, "mlp__alpha": alphas},
            {"mlp__hidden_layer_sizes": hidden_layers, "mlp__alpha": alphas},
        ]
        clf = GridSearchCV(
            pipe, param_grid=pipe_params, cv=3, scoring=scoring, return_train_score=True, verbose=verbose, n_jobs=n_jobs
        )
    ...
    if not greedy:
        curr_metric_dict = {
            curr_tuple: {
                "MLP r2": curr_mlp_r2,
                "linear r2": curr_lin_r2,
                "pow. law r2": curr_pl_r2,
                "mean r2": curr_mean_r2,
                "MLP RMSE": curr_mlp_rmse,
                "linear RMSE": curr_lin_rmse,
                "pow. law RMSE": curr_pl_rmse,
                "mean RMSE": curr_mean_rmse,
                ...
                "MLP MAPE": curr_mlp_mape,
                "linear MAPE": curr_lin_mape,
                "pow. law MAPE": curr_pl_mape,
                "mean MAPE": curr_mean_mape,
                "MLP rae": curr_mlp_rae,
                "linear rae": curr_lin_rae,
                "pow. law rae": curr_pl_rae,
                "mean rae": curr_mean_rae,
                "MLP dcor": curr_mlp_dcor,
                "linear dcor": curr_lin_dcor,
                "pow. law dcor": curr_pl_dcor,
                "mean dcor": curr_mean_dcor,
            }
        }
    else:
        curr_metric_dict = {
        ...
        if do_pl_fit:
            curr_data_dict = {
                curr_tuple: {
                    ...
                    "GridSearchParams": curr_best_params,
    ...
    logging.info(f"The analysis of this tuple took {str(round(time.time() - curr_start, 2))}s.")
    # for logging the progress of the analysis
    counter_tuples += 1
    print("-----" + str(counter_tuples) + "/" + str(len_data_tuples) + "-----")

    return curr_metric_dict, curr_data_dict, counter_tuples


@ray.remote(num_returns=3)
def parallel_pred_step_kNN(
    data,
    curr_tuple,
    input_cols,
    scaling,
    ...
    if scaling == "yes":
        ...
        pipe_params = [{"knn__n_neighbors": k_list}]
        ...
        pipe = Pipeline([("scaler", StandardScaler()), ("knn", KNeighborsRegressor())])
        pipe_params = [{"scaler": ["passthrough"], "knn__n_neighbors": k_list}]
        clf = GridSearchCV(
            pipe, param_grid=pipe_params, cv=3, scoring=scoring, return_train_score=True, verbose=verbose, n_jobs=n_jobs
        )
    else:
        pipe = Pipeline([("scaler", StandardScaler()), ("knn", KNeighborsRegressor())])
        pipe_params = [{"scaler": ["passthrough"], "knn__n_neighbors": k_list}, {"knn__n_neighbors": k_list}]
        clf = GridSearchCV(
            pipe, param_grid=pipe_params, cv=3, scoring=scoring, return_train_score=True, verbose=verbose, n_jobs=n_jobs
        )
    ...
    if not greedy:
        curr_metric_dict = {
            curr_tuple: {
                ...
                "pow. law RMSE/std": curr_pl_rmse_std,
                "mean RMSE/std": curr_mean_rmse / curr_y_test_std,
                "kNN MAPE": curr_knn_mape,
                "linear MAPE": curr_lin_mape,
                "pow. law MAPE": curr_pl_mape,
                "mean MAPE": curr_mean_mape,
                "kNN rae": curr_knn_rae,
                "linear rae": curr_lin_rae,
                "pow. law rae": curr_pl_rae,
                "mean rae": curr_mean_rae,
                "kNN dcor": curr_knn_dcor,
                "linear dcor": curr_lin_dcor,
                "pow. law dcor": curr_pl_dcor,
                "mean dcor": curr_mean_dcor,
            }
...
def refinement_step(data_dict, curr_tuple, time_left_for_this_task, generations, population_size, n_jobs):
    """
    ...
    # do data preparations and train-test-split
    curr_X_train, curr_X_test, curr_y_train, curr_y_test = (
        data_dict[curr_tuple]["X_train"],
...
def parallel_refinement_step(data_dict, curr_tuple, time_left_for_this_task, generations, population_size, n_jobs):
    """
    ...
    """
    curr_metric_dict, curr_data_dict = refinement_step(
        data_dict=data_dict,
...
def plot_result(input_datas_dict, plot_comb, refined_plot=False, initial_datas_dict=None, plot_along=[]):
    """
    ...
    :param refined_plot: boolean, default=False
        Specifies whether the plotting data includes results from a refined predictability run. Either because
    ...
    :param plot_along: list
        ...
        (subsets of) ["linear", "mean", "pl"] and ["init"] if ``refined_plot=True`` ("init" plots the initial
    ...
    if refined_plot:
        ...
        if initial_datas_dict:
            ...
            logging.error(
                "Cannot plot refined data as there's no refined datas dict given and no refined data is included in the initial datas dict."
            )
    # make dict a dataframe, name columns appropriately and compute error of kNN prediction
    results_df = pd.DataFrame([plotting_dict["y_test_pred"].flatten(), plotting_dict["y_test"].flatten()]).transpose()
    ...
    if plot_along:
        for comparison in plot_along:
            ...
            if (comparison == "pl") and ("y_test_pred_pl" not in plotting_dict.keys()):
                logging.error("no power law fit performed, some columns did not include positive values only")
            else:
    ...
    fig.add_trace(
        go.Scatter(
            x=results_df["true"], y=results_df["pred"], mode="markers", name=f"{main_plot} preds vs trues", opacity=0.8
        ),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Histogram(
            y=results_df["error"], nbinsx=int(np.floor(len(results_df["pred"]) / 10)), name=f"pred. error {main_plot}"
        ),
        row=1,
        col=3,
    )
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="pred", matches="y"),
