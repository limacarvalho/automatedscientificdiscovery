...
class SlugANN(BaseModel):
    ...
    def __objective__(self, trial, X_train, X_test, y_train, y_test):
        ...
        weighted_score = self.train_and_evaluate(params, model, trial, X_train, X_test, y_train, y_test)
    ...
    def train_and_evaluate(self, param, model, trial, X_train, X_test, y_train, y_test):
        ...
        train, test = TabularDataset(X_train, y_train, self.pred_class), TabularDataset(X_test, y_test, self.pred_class)
...
#                    print(y_test)
    ...
    def __predict__(self, model, X_train, X_test):
    ...
    def __discover_model__(self, X_train, X_test, y_train, y_test):
        ...
        study.optimize(obj_func, n_trials=self.n_trials, n_jobs=-1)
        ...
        pred_train, pred_test = self.__predict__(best_model, X_train, X_test)
        
        metirc_score_train, metirc_score_test, weighted_score = self.__get_model_score__(pred_train, pred_test, y_train, y_test, score_func=self.score_func)
    ...
    def load_score(self,  X_train, X_test, y_train, y_test, score_func=None, threshold=None):
        ...
        pred_train, pred_test = self.__predict__(self.model, X_train, X_test)
