...
def relevance(df, input_columns, target, options) -> None:
    ...
    if 'base_models' in options:
        ...
        result =  all(elem in list_ensemble_base_models for elem in base_models)
        if not result:
            raise ValueError("Pls select base_models from: " + str(list_ensemble_base_models))
    ...
    if 'attr_algos' in options:
        ...
        result =  all(elem in xai_attr_algos for elem in attr_algos)
        if not result:
    ...
    try:                
        ...
        if 'knockoff' in attr_algos:
            ...
            df_knockoffs = sim_knockoffs(param, knockoff_num_runs, df_X, df_y)
            
            
        ensemble_set = Ensemble(
                                list_base_models = base_models,     ### list of base models to fit the data
            
                                n_trials=n_trials,                   ### for all models
                                epochs=epochs,                       ### ANN param
                                boosted_round=boosted_round,         ### boosting tree param
                                max_depth=max_depth,                 ### boosting tree param
                                max_n_estimators=max_n_estimators,   ### rf param
                                n_estimators=n_estimators,           ### bagging param
                                n_neighbors=n_neighbors,             ### knn param
            
                                ### for ensemble model which is of bagging type
                                ensemble_n_estimators=ensemble_n_estimators,     
                                ensemble_n_trials=ensemble_n_trials,
                                        )
