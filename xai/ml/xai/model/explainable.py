    ...
    def get_attr(self, attr_algos):                
        ...
        for base_model in self.ensemble_set.base_models:            
            ...
            for attr_algo in attr_algos:
                if type(base_model.best_fit) == nn.Sequential:
                    if attr_algo == 'ig':
                        lazy_results.append(__get_ig_attr__.remote(base_model.best_fit, df_X_id, self.ig_n_steps))
                        col_names.append(base_model.model_file_name + '_' + attr_algo)
                    elif attr_algo == 'shap':          
                        lazy_results.append(__get_shapley_torch_attr__.remote(base_model.best_fit, df_X_id, self.shapley_n_background))
                        col_names.append(base_model.model_file_name + '_' + attr_algo)
                    elif attr_algo == 'gradientshap':
                        lazy_results.append(__get_gs_attr__.remote(base_model.best_fit, df_X_id, self.stdevs, self.n_samples))
                        col_names.append(base_model.model_file_name + '_' + attr_algo)
                elif (type(base_model.best_fit) in [RandomForestRegressor, RandomForestClassifier]) & (attr_algo == 'shap'):
                        lazy_results.append(__get_shapley_ensemble_attr__.remote(base_model.best_fit, df_X_id))
                        col_names.append(base_model.model_file_name + '_' + 'shap')

                elif (type(base_model.best_fit) in [KNeighborsRegressor, KNeighborsClassifier, BaggingClassifier, BaggingRegressor]) & (attr_algo == 'shap'):
                        lazy_results.append(__get_shapley_kernel_attr__.remote(base_model.best_fit, df_X_id, self.shapley_n_background))
                        col_names.append(base_model.model_file_name + '_' + 'shap')

                elif (type(base_model.best_fit) == xgb.core.Booster) & (attr_algo == 'shap'):
                        booster_type = json.loads(base_model.best_fit.save_config())['learner']['gradient_booster']['name']
                        if booster_type == 'gbtree':
                            lazy_results.append(__get_shapley_ensemble_attr__.remote(base_model.best_fit, df_X_id))
                            col_names.append(base_model.model_file_name + '_' + 'shap')
                    #else:
                    #    lazy_results.append(__get_shapley_kernel_attr__.remote(model, df_X_id, self.shapley_n_background))
