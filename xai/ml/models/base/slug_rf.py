...
class SlugRF(BaseModel):
    ...
    def __objective__(self, trial):
        ...
        params = {
            'n_estimators': trial.suggest_int('n_estimators', 50, self.max_n_estimators),
        ...
        if self.pred_class == 'regression':
            ...
            model = RandomForestRegressor(random_state=config.rand_state, **params)

        else:
            # cv = StratifiedKFold(n_splits=self.cv_splits, shuffle=True, random_state=config.rand_state)
            self.score_func = f1_score
            model =  RandomForestClassifier(random_state=config.rand_state, **params)
    ...
    def __discover_model__(self):
        ...
        customlogger.info( self.model_file_name + ': Starting training for trials:%d, max depth: %d, max estimators %d', self.n_trials, self.max_depth, self.max_n_estimators)
